...
use github::Github;
use crates::CratesIo;
use data::{InputCrateInfo, GeneratedCrateInfo, override_crate_data};
...
async fn main() -> Result<()> {
    ...
    let input: Vec<InputCrateInfo> = read_yaml(&path).with_context(|| format!("Error reading {}", path))?;
    ...
    for krate in input {
        ...
        let repo_opt: Option<Url> = gen.krate.as_ref().map(|k| k.repository.as_ref().map(|r| Url::parse(&r).unwrap())).flatten().clone();
        if let Some(repo) = repo_opt {
            if repo.host_str() == Some("github.com") {
                ...
                match gh.get_repo_data(parts[1], parts[2]).await {
                    ...
                    Err(err) => { 
                        eprintln!("Error getting Github repo data for {}/{} - {}", parts[1], parts[2], err)
                    },
                }
            }
