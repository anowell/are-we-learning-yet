...
pub fn override_crate_data(krate: &mut Crate, input: &InputCrateInfo) {
    ...
    replace_opt(&mut krate.repository, &input.repository.as_ref().map(|r| r.to_string()));
...
impl GeneratedCrateInfo {
    ...
    fn last_activity(&self) -> Option<DateTime<Utc>> {
        let mut last_activity = self.krate.as_ref().map(|k| k.updated_at).clone();
    ...
    pub fn update_score(&mut self) {
        ...
        let recent_downloads = self.krate.as_ref().and_then(|k| k.recent_downloads).unwrap_or(0);
